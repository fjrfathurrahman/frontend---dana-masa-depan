        ...
        <PopoverTrigger>
          <button>
            <Card className={`border-2 ${props.type === "deposit" ? "border-success bg-success/20" : "border-danger bg-danger/20"}`}>
              <CardBody className="flex flex-row items-center justify-between flex-shrink">
                <div className={`${props.type === "deposit" ? "bg-success" : "bg-danger"} me-2 flex min-h-8 min-w-8 flex-auto items-center justify-center rounded-full text-white`}>
                ...
                <h4 className="h4 w-max">{formatedCurrency(props.amount)}</h4>
              </CardBody>
            </Card>
          </button>
        </PopoverTrigger>
